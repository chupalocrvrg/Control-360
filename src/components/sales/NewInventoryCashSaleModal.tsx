...
import { collection, getDocs, addDoc, query, where, runTransaction, doc } from 'firebase/firestore';
import { X, User, ShoppingCart, DollarSign, Calendar, AlertCircle, Save, Package, Store, Hash, Check } from 'lucide-react';
...
export function NewInventoryCashSaleModal({
  ...
  // Cargar inventarios, bodegas y artículos
  useEffect(() => {
  ...
  // Cuando cambia el artículo seleccionado, autocompletar precio y verificar stock
  const selectedArticle = articles.find(a => a.id === selectedArticleId);
  const selectedWarehouse = warehouses.find(w => w.id === selectedWarehouseId);

  // Stock disponible en la bodega seleccionada para el artículo
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedArticleId) {
      showToast('Seleccione el artículo a vender.', 'warning');
      return;
    }
    if (!employeeId) {
    ...
    try {
      ...
      await runTransaction(db, async (transaction) => {
        ...
        // 3. Registrar venta en 'sales' (descontada del inventario)
  ...
  return (
    <div className="fixed inset-0 z-[120] bg-black/60 backdrop-blur-md flex items-center justify-center p-4">
      <div className="bg-white dark:bg-neutral-900 rounded-3xl p-6 sm:p-8 w-full max-w-xl shadow-2xl border border-neutral-200 dark:border-neutral-800 animate-in fade-in zoom-in-95 duration-200 max-h-[92vh] overflow-y-auto">
        ...
        <div className="flex justify-between items-start pb-4 mb-4 border-b border-neutral-100 dark:border-neutral-800">
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-neutral-400 font-medium">Descuenta Stock Directo</span>
            </div>
            ...
            <p className="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
              Seleccione la bodega y artículo para registrar la venta y rebajar las existencias automáticamente.
            </p>
          </div>
          <button 
            type="button" 
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  {selectedArticleId && (
                    <button
                      ...
                      title="Ingresar o consultar números de serie o chasis de este producto"
                    >
                      <Hash className="w-3 h-3" />
                      <span>{selectedArticle?.seriesList && selectedArticle.seriesList.length > 0 ? `Series (${selectedArticle.seriesList.length})` : '+ Ingresar Serie'}</span>
                    </button>
                  )}
                </div>
                <select
            ...
            {(selectedArticle?.requiresSeries || availableSeries.length > 0) && (
              <div className="p-4 bg-amber-50/70 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800/40 rounded-2xl space-y-3">
                ...
                {/* Alerta si faltan series por asignar */}
                {selectedSeries.length < quantity && (
                  <div className="flex items-center gap-2 text-[11px] text-amber-800 dark:text-amber-300 bg-amber-100/70 dark:bg-amber-900/30 px-3 py-1.5 rounded-xl border border-amber-200 dark:border-amber-800/40">
                    <AlertCircle className="w-3.5 h-3.5 flex-shrink-0 text-amber-600 dark:text-amber-400" />
                    <span>
                      Faltan {quantity - selectedSeries.length} serie(s) por asignar. Se permite concretar la venta y regularizar las series faltantes luego.
                    </span>
                  </div>
                )}
              </div>
            ...
            <div>
              ...
              />
              <datalist id="client-suggestions-cash">
                {clients.map(c => (
                  <option key={c.id} value={`${c.name} ${c.lastName || ''}`.trim()} />
                ))}
              </datalist>
            </div>
