...
export default function SalesTab() {
  ...
  const handleSubmit = async (e: React.FormEvent, bypassNegativeWarning = false) => {
    e.preventDefault();
    setError('');
    setSuccess('');
    ...
    // Check quantities and identify items that will go negative (< 0)
    ...
    for (const item of saleItems) {
      ...
      const projected = available - stockChecks[key];
    ...
    // If stock will go negative and user hasn't explicitly confirmed yet, pop modal!
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
      ...
      <NegativeStockWarningModal
        ...
        onConfirmProceed={() => {
          ...
          handleSubmit(dummyEvent, true);
        }}
        onCorrection={() => {
          setNegativeStockModalOpen(false);
          showToast('Corrija la fila de artículos o seleccione otro ítem.', 'info');
        }}
      />
    </div>
  );
