...
import NegativeStockWarningModal, { NegativeStockItemInfo } from './NegativeStockWarningModal';
...
export default function TransfersTab() {
  ...
  // Negative Stock Warning Modal States
  const [negativeStockModalOpen, setNegativeStockModalOpen] = useState(false);
  const [negativeItemsWarning, setNegativeItemsWarning] = useState<NegativeStockItemInfo[]>([]);
  ...
  const handleTransferSubmit = async (e: React.FormEvent, bypassNegativeWarning = false) => {
    e.preventDefault();
    setError('');
    setSuccess('');
    ...
    // Validate quantities and identify items that will go negative (< 0)
    const itemsGoingNegative: NegativeStockItemInfo[] = [];
    const requestedStockMap: { [articleId: string]: number } = {};
    ...
    for (const item of transferItems) {
      ...
      requestedStockMap[item.articleId] = (requestedStockMap[item.articleId] || 0) + item.quantity;
    }

    for (const [articleId, totalRequested] of Object.entries(requestedStockMap)) {
      const available = getAvailableStock(articleId, fromWarehouseId);
      const projected = available - totalRequested;

      if (projected < 0) {
        const art = articles.find(a => a.id === articleId);
        const whName = warehouses.find(w => w.id === fromWarehouseId)?.name || 'Bodega Origen';
        itemsGoingNegative.push({
          articleId: articleId,
          articleName: art?.name || 'Artículo',
    ...
    // Pop warning modal if items will go negative and hasn't been confirmed
    if (itemsGoingNegative.length > 0 && !bypassNegativeWarning) {
      setNegativeItemsWarning(itemsGoingNegative);
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
      ...
      <NegativeStockWarningModal
        isOpen={negativeStockModalOpen}
        onClose={() => setNegativeStockModalOpen(false)}
        negativeItems={negativeItemsWarning}
        onConfirmProceed={() => {
          setNegativeStockModalOpen(false);
          const dummyEvent = { preventDefault: () => {} } as React.FormEvent;
          handleTransferSubmit(dummyEvent, true);
        }}
        onCorrection={() => {
          setNegativeStockModalOpen(false);
          showToast('Corrija la cantidad o cambie de ítem.', 'info');
        }}
      />
    </div>
  );
