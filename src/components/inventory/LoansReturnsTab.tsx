...
import { QuickNewArticleModal } from './QuickNewArticleModal';
...
export default function LoansReturnsTab() {
  ...
  // Calculate borrowed units pending to return to this commercial house (LOAN = borrow, RETURN = give back)
  const houseBorrowedDebt = React.useMemo(() => {
    const debt: Record<string, number> = {};
    if (commercialHouse) {
      logs.filter(lr => lr.status !== 'ELIMINADO' && lr.commercialHouse.trim().toLowerCase() === commercialHouse.trim().toLowerCase()).forEach(lr => {
        lr.articles.forEach(art => {
          if (!debt[art.articleId]) debt[art.articleId] = 0;
          if (lr.type === 'LOAN') {
            debt[art.articleId] += art.quantity;
    ...
    return debt;
  }, [commercialHouse, logs]);

  // For LOAN (Pedir prestado): All articles in the catalog are available, especially those with 0 stock!
  // For RETURN (Devolver): All articles can be viewed, but ArticleSelector and handleSubmit ensure stock > 0 in the selected warehouse.
  const availableArticles = articles;
  const effectiveInventories = inventories;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validate quantities and stock availability for RETURNS / LOANS (aggregated by articleId)
    const totalRequestedByArticle: { [articleId: string]: number } = {};
    for (const item of items) {
      ...
      totalRequestedByArticle[item.articleId] = (totalRequestedByArticle[item.articleId] || 0) + item.quantity;
    }

    for (const [articleId, totalQty] of Object.entries(totalRequestedByArticle)) {
      const articleName = articles.find(a => a.id === articleId)?.name || 'Artículo';
      if (type === 'RETURN') { // Egreso / Devolución hacia casa comercial
        const availableInWarehouse = inventories
          .filter(inv => inv.warehouseId === warehouseId && inv.articleId === articleId)
          .reduce((acc, inv) => acc + inv.quantity, 0);

        if (availableInWarehouse <= 0) {
          setError(`No puede devolver "${articleName}" porque tiene 0 existencias en la bodega seleccionada.`);
          return;
        }

        if (totalQty > availableInWarehouse) {
          setError(`Stock insuficiente en bodega para devolver "${articleName}". Solicitado: ${totalQty} uds, Disponible en bodega: ${availableInWarehouse} uds.`);
          return;
        }
      }
      // Note: When type === 'LOAN', we are borrowing merchandise into our warehouse. 
      // It is completely valid for current stock to be 0 (that is why we borrow it). The quantity will be added to the warehouse.
    }
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
      ...
      <div className="lg:col-span-5 bg-white dark:bg-neutral-900 rounded-[2.5rem] border border-neutral-200 dark:border-neutral-800 p-8 shadow-sm h-fit">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            {type === 'LOAN' ? (
              <p className="text-[11px] text-indigo-700 dark:text-indigo-300 mt-2 font-medium bg-indigo-50 dark:bg-indigo-950/30 p-2.5 rounded-xl border border-indigo-100 dark:border-indigo-800/40">
                <strong>Ingreso (Pedir Prestado):</strong> Ingresa artículos de una casa comercial a tu bodega. Puedes seleccionar artículos con stock en 0 o crearlos al vuelo. La cantidad ingresada se sumará a tu stock.
              </p>
            ) : (
              <p className="text-[11px] text-amber-700 dark:text-amber-300 mt-2 font-medium bg-amber-50 dark:bg-amber-950/30 p-2.5 rounded-xl border border-amber-100 dark:border-amber-800/40">
                <strong>Egreso (Devolución):</strong> Devuelve artículos hacia la casa comercial para saldar deuda de préstamo. Requiere existencias físicas en la bodega seleccionada (no se permite stock en 0).
              </p>
            )}
          </div>
                  ...
                  <div key={index} className="flex gap-2 items-start bg-neutral-50 dark:bg-neutral-800/30 p-3 rounded-2xl border border-neutral-100 dark:border-neutral-800/50">
                    <div className="flex-1">
                      <ArticleSelector 
                        ...
                        isReceiving={type === 'LOAN'} // When receiving a loan from commercial house, allows creating or picking items even with 0 stock
                        maxQuantityStr={type === 'RETURN' && item.articleId && houseBorrowedDebt[item.articleId] ? `(Deuda con esta casa: ${houseBorrowedDebt[item.articleId]} uds)` : undefined}
                        onQuickCreate={(searchText) => {
      ...
      {/* Modal para crear nuevo artículo al vuelo */}
      <QuickNewArticleModal
        ...
        onArticleCreated={async (newArticle) => {
          await fetchData();
          if (quickCreateRowIndex !== null && quickCreateRowIndex >= 0) {
            handleItemChange(quickCreateRowIndex, 'articleId', newArticle.id);
          }
        }}
      />
    </div>
