...
export default function InventoryDashboard() {
  ...
  const negativeStockArticles = articles.filter(art => art.quantity < 0);
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      {negativeStockArticles.length > 0 && (
        <div className="p-5 bg-gradient-to-r from-rose-50 to-amber-50 dark:from-rose-950/40 dark:to-amber-950/30 border border-rose-200 dark:border-rose-900/60 rounded-3xl flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-xs font-black text-rose-800 dark:text-rose-300 block uppercase tracking-wider">
                Alerta de Stock Negativo ({negativeStockArticles.length} artículos)
              </span>
              <span className="text-xs text-rose-600/90 dark:text-rose-400 font-medium">
                Se detectaron mercancías despachadas o vendidas con saldo menor a cero.
              </span>
            </div>
          </div>
          <div className="flex items-center gap-2">
            {negativeStockArticles.slice(0, 3).map(art => (
              <span key={art.id} className="text-[10px] font-bold px-2.5 py-1 bg-rose-100 dark:bg-rose-900/50 text-rose-700 dark:text-rose-300 rounded-xl">
                {art.name}: {art.quantity} uds
              </span>
            ))}
            {negativeStockArticles.length > 3 && (
              <span className="text-[10px] font-bold text-rose-500">+{negativeStockArticles.length - 3} más</span>
            )}
          </div>
        </div>
      )}
