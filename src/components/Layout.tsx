    ...
    : [
        ...
        },
        {
          ...
          subItems: [
            ...
            { name: 'Gestión de Carteras', href: '/portfolio', icon: FolderGit2 },
  ...
  return (
    <div className={containerClass}>
      ...
      <div 
        ...
        onMouseLeave={() => setHoveredIndex(null)}
      >
        {/* Logo item in dock - Click to navigate to Dashboard */}
        <Link
          to="/"
          title="Ir al Dashboard"
          className={cn(
            "relative group flex items-center justify-center h-12 w-12 rounded-2xl border transition-all duration-300 cursor-pointer hover:scale-105 active:scale-95",
            isClassicStyle
              ? "bg-white dark:bg-neutral-800 border-neutral-200 dark:border-neutral-700 shadow-sm hover:bg-neutral-50 dark:hover:bg-neutral-700"
              : "bg-white/20 dark:bg-black/20 border border-white/20 backdrop-blur-sm shadow-sm hover:bg-white/30 dark:hover:bg-black/30"
          )}
        >
          <img src="/logo.svg" alt="Control 360°" className="h-7 w-7 select-none" referrerPolicy="no-referrer" />
          <div className="absolute bottom-full mb-3 left-1/2 -translate-x-1/2 px-2.5 py-1 text-[11px] font-bold text-white bg-neutral-900/85 dark:bg-black/90 rounded-lg opacity-0 group-hover:opacity-100 transition-all pointer-events-none whitespace-nowrap z-50 border border-white/10 shadow-lg">
            Ir al Dashboard
          </div>
        </Link>
      ...
      <div className={cn(
        ...
        <Link to="/" className="flex items-center cursor-pointer group">
          <div className="h-8 w-8 bg-indigo-600 dark:bg-indigo-500 rounded-lg flex items-center justify-center mr-2 group-hover:scale-105 transition-transform">
      ...
      {/* Main content */}
      <div className={cn(
        "flex-1 flex flex-col h-screen overflow-x-hidden overscroll-y-none relative z-10 transition-all duration-300 pb-36 md:pb-0",
        hasActiveModal ? "overflow-hidden" : "overflow-y-auto",
        pos === 'left' && "md:pl-32",
        pos === 'right' && "md:pr-32",
        pos === 'top' && "pt-32",
        pos === 'bottom' && "pb-48 md:pb-56"
      )}>
        {impersonatedUser && (
