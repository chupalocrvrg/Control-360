...
import { MessageCircle, Lock, ShieldAlert, Power, ShieldCheck, Fingerprint, Clock, AlertTriangle, Check } from 'lucide-react';
...
export default function SecurityGuard({ children }: { children: React.ReactNode }) {
  const { isExpired, sessionVerified, verifyPin, logout, profile, setProfile, updateProfile, setSessionVerified, impersonatedUser, loading: authLoading } = useAuth();
  ...
  // Terms Acceptance & Onboarding states
  ...
  // Check acceptance (bypass if SuperAdmin is impersonating this user)
  const isDerick = profile?.email === 'creditosderick15@gmail.com';
  const needsContract = isDerick && !profile?.contractAcceptedAt;
  const needsTerms = profile?.termsVersion !== TERMS_VERSION_CURRENT;
  ...
  const handleAcceptTerms = async () => {
    ...
    try {
      ...
      if (profile?.email === 'creditosderick15@gmail.com') {
        updates.contractAcceptedAt = new Date().toISOString();
      }

      await updateProfile(updates);
      
      // Marcar como aceptado localmente para evitar el bucle de sincronización
      setLocallyAccepted(true);
      
      // Sincronización proactiva del perfil para evitar el rebote por latencia de Firestore
      ...
      // En lugar de cerrar todo, activamos el Tour de Bienvenida
      setShowTour(true);
    } catch (error) {
  ...
  const handleFinishTour = () => {
    ...
    // Mantener bypass por 3 segundos para dar tiempo a la sincronización de Firebase
    setTimeout(() => {
  ...
  if (showTour) {
    return <OnboardingTour onFinish={handleFinishTour} />;
  }

  // Prioridad: Si está verificado pero necesita documentos, mostrar modal LEGAL
  // Esto evita el flicker de "Acceso Limitado" y el uso de locallyAccepted evita el bucle
  if (!authLoading && sessionVerified && !impersonatedUser && !isTransitioning && !locallyAccepted && (needsTerms || needsContract || showTerms)) {
    return <TermsAcceptanceModal onAccept={handleAcceptTerms} />;
  }

  // Bloqueo solo si hay un perfil, no estamos en transición y no estamos impersonando
  const isBlocked = !authLoading && !!profile && !impersonatedUser && !isTransitioning && !locallyAccepted && (needsContract || needsTerms);

  if (isBlocked) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-neutral-50 dark:bg-neutral-950 p-6">
        <div className="max-w-md w-full bg-white dark:bg-neutral-900 p-8 rounded-3xl border border-neutral-200 dark:border-neutral-800 text-center space-y-6">
          <AlertTriangle className="w-16 h-16 text-amber-500 mx-auto" />
          <h2 className="text-xl font-bold text-neutral-900 dark:text-white">Acceso Limitado</h2>
          <p className="text-sm text-neutral-600 dark:text-neutral-400">
            Los términos, condiciones y contrato (si aplica) no han sido aceptados. Por seguridad, no puedes utilizar ninguna función del sistema.
          </p>
          <button 
