...
import { getDefaultEnterpriseUid, syncLinkedUserToEmployees } from '../lib/enterprise';
...
export default function AdminUsers({ mode = "USERS" }: { mode?: "USERS" | "HISTORY" | "AUDIT" | "TRASH" | "ENTITIES" }) {
  ...
  const handleOpenWizard = async () => {
    ...
    const defaultEnterpriseId = await getDefaultEnterpriseUid();
    setWizardTargetEnterpriseId(defaultEnterpriseId);
  ...
  const getDocsToDeleteForUser = async (userToClear: any) => {
    // Strictly isolate to ONLY user-specific IDs (never enterpriseId or company-wide IDs)
    const targetUserIds = new Set<string>();
    if (userToClear.id && typeof userToClear.id === 'string') targetUserIds.add(userToClear.id.trim());
    if (userToClear.uid && typeof userToClear.uid === 'string') targetUserIds.add(userToClear.uid.trim());

    const idsArr = Array.from(targetUserIds).filter(id => id.length > 0);
    console.log("IDs de usuario estrictamente aislados para limpieza individual:", idsArr);

    if (idsArr.length === 0) {
      return new Map<string, { colName: string; docId: string }>();
    }
    ...
    // Strictly check user ownership fields (NEVER enterpriseId)
    const userOwnershipFields = ['userId', 'createdBy'];
    const docsToDeleteMap = new Map<string, { colName: string; docId: string }>();

    const queryPromises: Promise<any>[] = [];
    for (const colName of collectionsToCheck) {
      for (const field of userOwnershipFields) {
        for (const uId of idsArr) {
          ...
          queryPromises.push(
            getDocs(q).then(snap => {
              snap.docs.forEach(d => {
                const data = d.data();
                // Safety guard: ensure the document is actually created by/belongs to this user
                if (data.userId === uId || data.createdBy === uId) {
                  docsToDeleteMap.set(`${colName}/${d.id}`, { colName, docId: d.id });
                }
              });
            }).catch((err) => {
              console.warn(`Query error checking ${colName}.${field}:`, err);
            })
          );
        }
      }
