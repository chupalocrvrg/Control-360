...
import { 
  ...
  ChevronRight, ArrowUpRight, TrendingUp, TrendingDown, DollarSign, BarChart3, PieChart,
  Briefcase, Layers, Users, Eye
} from 'lucide-react';
...
import { PortfolioSnapshot, AGING_BRACKETS } from '../types/portfolio';
...
interface CommerceData {
  ...
  salesContadoVal?: number;
  salesCreditoVal?: number;
  motosContadoVal?: number;
  motosCreditoVal?: number;
  portfolioEvolution?: CollectorPortfolioEvolution;
}
...
export default function Dashboard() {
  ...
  const [monthlyPortfolioCutoffs, setMonthlyPortfolioCutoffs] = useState<{
    initialSnapshot: PortfolioSnapshot | null;
    closingSnapshot: PortfolioSnapshot | null;
    initialDate: string;
    closingDate: string;
  }>({ initialSnapshot: null, closingSnapshot: null, initialDate: '', closingDate: '' });

  // On-screen preview state for General Report (Option C)
  const [reportPreviewData, setReportPreviewData] = useState<{
    empCobros: any[];
    portfolioRows: any[];
    cutoffs: { initialDate: string; closingDate: string };
    totalInit: number;
    totalClose: number;
    totalRecov: number;
    totalVarPct: string;
    totalInitOnTime: number;
    totalCloseOnTime: number;
    totalInitOverdue: number;
    totalCloseOverdue: number;
  } | null>(null);
  ...
  const loadDashboardData = async () => {
    ...
    try {
      // 1. Fetch all data in parallel to boost speed
      const [snapshot, empSnap, budgetSnap, allSalesSnap, allCollSnap, portfolioSnap] = await Promise.all([
        getDocs(query(collection(db, path), where('enterpriseId', '==', currentEnterpriseId))),
        getDocs(query(collection(db, 'employees'), where('enterpriseId', '==', currentEnterpriseId))),
        getDocs(query(collection(db, 'budgets'), where('enterpriseId', '==', currentEnterpriseId))),
        getDocs(query(collection(db, 'sales'), where('enterpriseId', '==', currentEnterpriseId))),
        getDocs(query(collection(db, 'collections'), where('enterpriseId', '==', currentEnterpriseId))),
        getDocs(query(collection(db, 'portfolio_snapshots'), where('enterpriseId', '==', currentEnterpriseId)))
      ]);

      // Process portfolio snapshots and monthly cutoffs
      const allSnapshots = portfolioSnap.docs.map(d => ({ id: d.id, ...d.data() } as PortfolioSnapshot));
      setPortfolioSnapshots(allSnapshots);
      const cutoffs = getMonthlyPortfolioCutoffs(allSnapshots, currentMonth);
      setMonthlyPortfolioCutoffs(cutoffs);
      ...
      const commerceArray = employees.map(emp => {
        ...
        const portfolioEvolution = getCollectorPortfolioEvolution(
          cutoffs.initialSnapshot,
          cutoffs.closingSnapshot,
          fullName,
          isSupervisor,
          cutoffs.initialDate,
          cutoffs.closingDate
      ...
      setAllCommerceData(commerceArray.filter(c => 
        c.salesBudget > 0 || 
        c.collectionsBudget > 0 || 
        c.portfolioEvolution?.hasData || 
        ['cobrador', 'ambos', 'supervisor_cobranza', 'supervisor_general'].includes(c.employee.role)
      ));
  ...
  const monthlyData: Array<{ month: string; total: number; paid: number; pending: number; ventas: number; cobros: number }> = [];
...
const handleGenerateAdvancedReport = async (reportType: 'pdf' | 'excel' | 'preview') => {
    ...
    try {
      ...
      const [checksSnap, salesSnap, collSnap, empSnap, budgetSnap, portfolioSnap] = await Promise.all([
        getDocs(query(collection(db, 'checks'), where('enterpriseId', '==', currentEnterpriseId))),
        getDocs(query(collection(db, 'sales'), where('enterpriseId', '==', currentEnterpriseId))),
        getDocs(query(collection(db, 'collections'), where('enterpriseId', '==', currentEnterpriseId))),
        getDocs(query(collection(db, 'employees'), where('enterpriseId', '==', currentEnterpriseId))),
        getDocs(query(collection(db, 'budgets'), where('enterpriseId', '==', currentEnterpriseId))),
      ...
      // Process Portfolio Snapshots according to cutoff rule:
      // First cutoff loaded in the period (or marked as initial) as Start, and last cutoff loaded as End
      const periodSnapshots = allSnapshots.filter(s => {
        const sDate = s.cutoffDate || (s.month ? `${s.month}-01` : '');
        return (sDate >= reportStartDate && sDate <= reportEndDate) || s.month === startMonthStr || s.month === endMonthStr;
      });

      const reportCutoffs = getMonthlyPortfolioCutoffs(
        periodSnapshots.length > 0 ? periodSnapshots : allSnapshots,
        startMonthStr === endMonthStr ? startMonthStr : undefined
      );
      ...
      let totalMotosContadoCombVal = 0;
      let totalMotosContadoCombUds = 0;
      let totalMotosContadoElecVal = 0;
      let totalMotosContadoElecUds = 0;

      let totalMotosCreditoCombVal = 0;
      let totalMotosCreditoCombUds = 0;
      let totalMotosCreditoElecVal = 0;
      let totalMotosCreditoElecUds = 0;
      ...
      employees.forEach((emp: any) => {
        empSummary[emp.id] = { 
          ...
          motosContCombUds: 0, motosContElecUds: 0,
          motosCredCombUds: 0, motosCredElecUds: 0,
          motosContVal: 0, motosCredVal: 0,
          collections: 0, 
      ...
      salesData.forEach(s => {
        ...
        const isElectrico = s.motoType === 'electrico';
        if (s.isMoto) {
          if (s.type === 'contado') {
            if (isElectrico) {
              totalMotosContadoElecVal += val;
              totalMotosContadoElecUds += 1;
              empSummary[s.employeeId].motosContElecUds += 1;
            } else {
              totalMotosContadoCombVal += val;
              totalMotosContadoCombUds += 1;
              empSummary[s.employeeId].motosContCombUds += 1;
            }
            empSummary[s.employeeId].motosContVal += val;
          } else {
            if (isElectrico) {
              totalMotosCreditoElecVal += val;
              totalMotosCreditoElecUds += 1;
              empSummary[s.employeeId].motosCredElecUds += 1;
            } else {
              totalMotosCreditoCombVal += val;
              totalMotosCreditoCombUds += 1;
              empSummary[s.employeeId].motosCredCombUds += 1;
            }
      ...
      const totalMotosContadoUds = totalMotosContadoCombUds + totalMotosContadoElecUds;
      const totalMotosContadoVal = totalMotosContadoCombVal + totalMotosContadoElecVal;
      const totalMotosCreditoUds = totalMotosCreditoCombUds + totalMotosCreditoElecUds;
      const totalMotosCreditoVal = totalMotosCreditoCombVal + totalMotosCreditoElecVal;
      const totalMotosCombUds = totalMotosContadoCombUds + totalMotosCreditoCombUds;
      const totalMotosCombVal = totalMotosContadoCombVal + totalMotosCreditoCombVal;
      const totalMotosElecUds = totalMotosContadoElecUds + totalMotosCreditoElecUds;
      const totalMotosElecVal = totalMotosContadoElecVal + totalMotosCreditoElecVal;
      const totalMotosAllUds = totalMotosCombUds + totalMotosElecUds;
      const totalMotosAllVal = totalMotosCombVal + totalMotosElecVal;
      ...
      // -------------------------------------------------------------
      // EVOLUCIÓN DE CARTERA POR COBRADOR (INICIO VS FIN DE MES)
      // -------------------------------------------------------------
      const collectorMap = new Map<string, { isSupervisor: boolean; name: string }>();
      employees.forEach((emp: any) => {
        const isSuper = emp.role && emp.role.startsWith('supervisor');
        const isColl = ['cobrador', 'ambos', 'supervisor_cobranza', 'supervisor_general'].includes(emp.role);
        const hasCollAct = empSummary[emp.id] && (empSummary[emp.id].collections > 0 || empSummary[emp.id].collBudget > 0);
        if (isColl || hasCollAct) {
          const fullName = `${emp.name || ''} ${emp.lastName || ''}`.trim();
          collectorMap.set(fullName, { isSupervisor: isSuper, name: fullName });
        }
      });

      if (reportCutoffs.initialSnapshot?.collectorSummary) {
        Object.keys(reportCutoffs.initialSnapshot.collectorSummary).forEach(name => {
          if (!collectorMap.has(name)) collectorMap.set(name, { isSupervisor: false, name });
        });
      }
      if (reportCutoffs.closingSnapshot?.collectorSummary) {
        Object.keys(reportCutoffs.closingSnapshot.collectorSummary).forEach(name => {
          if (!collectorMap.has(name)) collectorMap.set(name, { isSupervisor: false, name });
        });
      }
      ...
      let totalInit = 0;
      let totalClose = 0;
      let totalInitOnTime = 0;
      let totalCloseOnTime = 0;
      let totalInitOverdue = 0;
      let totalCloseOverdue = 0;

      const portfolioReportRows: any[] = [];
      const excelPortfolioRows: any[] = [];
      const previewPortfolioRows: any[] = [];

      Array.from(collectorMap.values()).forEach(({ name, isSupervisor }) => {
        const evo = getCollectorPortfolioEvolution(
          reportCutoffs.initialSnapshot,
          reportCutoffs.closingSnapshot,
          name,
          isSupervisor,
          reportCutoffs.initialDate,
          reportCutoffs.closingDate
        );

        if (!evo.hasData && !isSupervisor) {
          const hasCobro = Object.values(empSummary).some(e => e.name === name && (e.collections > 0 || e.collBudget > 0));
          if (!hasCobro) return;
        }

        if (!isSupervisor) {
          totalInit = round2(totalInit + evo.initialAmount);
          totalClose = round2(totalClose + evo.closingAmount);
          totalInitOnTime = round2(totalInitOnTime + evo.initialOnTimeAmount);
          totalCloseOnTime = round2(totalCloseOnTime + evo.closingOnTimeAmount);
          totalInitOverdue = round2(totalInitOverdue + evo.initialOverdueAmount);
          totalCloseOverdue = round2(totalCloseOverdue + evo.closingOverdueAmount);
        }

        const recovOrVar = evo.variationAmount <= 0
          ? `-$${evo.recoveredAmount.toFixed(2)}`
          : `+$${evo.variationAmount.toFixed(2)}`;
        const varPctStr = evo.initialAmount > 0
          ? `${evo.variationPct >= 0 ? '+' : ''}${evo.variationPct.toFixed(1)}%`
          : '0%';

        portfolioReportRows.push([
          name + (isSupervisor ? ' (Superv.)' : ''),
          evo.initialAmount.toFixed(2),
          evo.closingAmount.toFixed(2),
          recovOrVar,
          varPctStr,
          evo.initialOnTimeAmount.toFixed(2),
          evo.closingOnTimeAmount.toFixed(2),
          evo.initialOverdueAmount.toFixed(2),
          evo.closingOverdueAmount.toFixed(2)
        ]);

        excelPortfolioRows.push([
          name + (isSupervisor ? ' (Supervisor)' : ''),
          evo.initialAmount,
          evo.closingAmount,
          evo.variationAmount <= 0 ? -evo.recoveredAmount : evo.variationAmount,
          varPctStr,
          evo.initialOnTimeAmount,
          evo.closingOnTimeAmount,
          evo.initialOverdueAmount,
          evo.closingOverdueAmount,
          evo.initialBrackets['0']?.totalAmount || 0,
          evo.initialBrackets['1-30']?.totalAmount || 0,
          evo.initialBrackets['31-60']?.totalAmount || 0,
          evo.initialBrackets['61-90']?.totalAmount || 0,
          evo.initialBrackets['91-180']?.totalAmount || 0,
          evo.initialBrackets['>181']?.totalAmount || 0,
          evo.closingBrackets['0']?.totalAmount || 0,
          evo.closingBrackets['1-30']?.totalAmount || 0,
          evo.closingBrackets['31-60']?.totalAmount || 0,
          evo.closingBrackets['61-90']?.totalAmount || 0,
          evo.closingBrackets['91-180']?.totalAmount || 0,
          evo.closingBrackets['>181']?.totalAmount || 0
        ]);

        previewPortfolioRows.push({
          name: name + (isSupervisor ? ' (Superv.)' : ''),
          initialAmount: evo.initialAmount,
          closingAmount: evo.closingAmount,
          variationAmount: evo.variationAmount,
          recoveredAmount: evo.recoveredAmount,
          variationPct: evo.variationPct,
          initialOnTimeAmount: evo.initialOnTimeAmount,
          closingOnTimeAmount: evo.closingOnTimeAmount,
          initialOverdueAmount: evo.initialOverdueAmount,
          closingOverdueAmount: evo.closingOverdueAmount,
          hasData: evo.hasData
        });
      });

      if (totalInit === 0 && reportCutoffs.initialSnapshot) {
        totalInit = reportCutoffs.initialSnapshot.totalAmount;
        totalInitOnTime = round2((reportCutoffs.initialSnapshot.bracketSummary['0']?.totalAmount || 0) + (reportCutoffs.initialSnapshot.bracketSummary['1-30']?.totalAmount || 0));
        totalInitOverdue = round2(totalInit - totalInitOnTime);
      }
      if (totalClose === 0 && reportCutoffs.closingSnapshot) {
        totalClose = reportCutoffs.closingSnapshot.totalAmount;
        totalCloseOnTime = round2((reportCutoffs.closingSnapshot.bracketSummary['0']?.totalAmount || 0) + (reportCutoffs.closingSnapshot.bracketSummary['1-30']?.totalAmount || 0));
        totalCloseOverdue = round2(totalClose - totalCloseOnTime);
      }

      const totalRecov = Math.max(0, round2(totalInit - totalClose));
      const totalVarPct = totalInit > 0 ? round2(((totalClose - totalInit) / totalInit) * 100).toFixed(1) + '%' : '0%';

      // -------------------------------------------------------------
      // VISTA PREVIA EN PANTALLA (Opción C)
      // -------------------------------------------------------------
      if (reportType === 'preview') {
        const empCobrosList = Object.values(empSummary).filter(e => e.collections > 0 || e.collBudget > 0).map(e => {
          const pctC = e.collBudget > 0 ? ((e.collections / e.collBudget) * 100).toFixed(1) + '%' : '0%';
          return {
            name: e.name,
            collections: e.collections,
            collBudget: e.collBudget,
            pctC
          };
        });

        setReportPreviewData({
          empCobros: empCobrosList,
          portfolioRows: previewPortfolioRows,
          cutoffs: { initialDate: reportCutoffs.initialDate, closingDate: reportCutoffs.closingDate },
          totalInit,
          totalClose,
          totalRecov,
          totalVarPct,
      ...
      if (reportType === 'pdf') {
        ...
        doc.setFontSize(13);
        doc.text("Cuadro 2: Reporte General de Ventas", 14, (doc as any).lastAutoTable.finalY + 12);
        autoTable(doc, {
          startY: (doc as any).lastAutoTable.finalY + 16,
          head: [["Concepto", "Combustión (Uds)", "Eléctricas (Uds)", "Total (Uds)", "Monto ($)", "Presupuesto ($)", "% Cump."]],
          body: [
            ["Ventas Contado (Artículos)", "-", "-", "-", totalArticulosContado.toFixed(2), "-", "-"],
            ["Ventas Contado (Motos)", totalMotosContadoCombUds.toString(), totalMotosContadoElecUds.toString(), totalMotosContadoUds.toString(), totalMotosContadoVal.toFixed(2), "-", "-"],
            ["Ventas Crédito (Artículos)", "-", "-", "-", totalArticulosCredito.toFixed(2), "-", "-"],
            ["Ventas Crédito (Motos)", totalMotosCreditoCombUds.toString(), totalMotosCreditoElecUds.toString(), totalMotosCreditoUds.toString(), totalMotosCreditoVal.toFixed(2), "-", "-"],
            ["Total General Motos", totalMotosCombUds.toString(), totalMotosElecUds.toString(), totalMotosAllUds.toString(), totalMotosAllVal.toFixed(2), "-", "-"],
        ...
        // Flujo continuo justo después del Cuadro 2
        doc.setFontSize(13);
        doc.text("Balance de Ventas por Vendedor", 14, (doc as any).lastAutoTable.finalY + 12);
        ...
        const empVentas = Object.values(empSummary).filter(e => e.artContado > 0 || e.artCredito > 0 || e.motosContVal > 0 || e.motosCredVal > 0 || e.salesBudget > 0).map(e => {
          ...
          const mCombUds = e.motosContCombUds + e.motosCredCombUds;
          const mElecUds = e.motosContElecUds + e.motosCredElecUds;
          const totalMotosUds = mCombUds + mElecUds;
          return [
            e.name,
            mCombUds.toString(),
            mElecUds.toString(),
        ...
        autoTable(doc, {
          startY: (doc as any).lastAutoTable.finalY + 16,
          head: [["Vendedor", "Motos Comb. (uds)", "Motos Eléc. (uds)", "Total Motos (uds)", "Total Artículos ($)", "Presupuesto ($)", "% Cump."]],
          body: empVentas.length > 0 ? empVentas : [["Sin ventas en el periodo", "-", "-", "-", "-", "-", "-"]],
          theme: 'grid',
          headStyles: { fillColor: [79, 70, 229] }
        });

        doc.setFontSize(13);
        doc.text("Balance de Cobranzas por Cobrador", 14, (doc as any).lastAutoTable.finalY + 12);
        ...
        autoTable(doc, {
          ...
          body: empCobros.length > 0 ? empCobros : [["Sin cobros en el periodo", "-", "-", "-"]],
          theme: 'grid',
          headStyles: { fillColor: [14, 165, 233] }
        });

        // Continuación solicitada: Evolución de Cartera por Cobrador (Inicio de Mes vs. Fin de Mes)
        doc.setFontSize(13);
        doc.text("Evolución de Cartera por Cobrador (Inicio de Mes vs. Fin de Mes)", 14, (doc as any).lastAutoTable.finalY + 12);
        doc.setFontSize(8.5);
        doc.text(`Cortes analizados: ${reportCutoffs.initialDate || 'Inicio'} al ${reportCutoffs.closingDate || 'Fin de Mes'}`, 14, (doc as any).lastAutoTable.finalY + 16);

        autoTable(doc, {
          startY: (doc as any).lastAutoTable.finalY + 19,
          head: [["Cobrador", "Cartera Inicio ($)", "Cartera Fin ($)", "Recup./Var. ($)", "% Var.", "Inicio Al Día ($)", "Fin Al Día ($)", "Inicio Venc. ($)", "Fin Venc. ($)"]],
          body: portfolioReportRows.length > 0 ? portfolioReportRows : [["Sin cortes de cartera registrados", "-", "-", "-", "-", "-", "-", "-", "-"]],
          foot: portfolioReportRows.length > 0 ? [[
            "TOTAL CONSOLIDADO",
            totalInit.toFixed(2),
            totalClose.toFixed(2),
            totalRecov.toFixed(2),
            totalVarPct,
            totalInitOnTime.toFixed(2),
            totalCloseOnTime.toFixed(2),
            totalInitOverdue.toFixed(2),
            totalCloseOverdue.toFixed(2)
          ]] : undefined,
          theme: 'grid',
        ...
        const sheetRows: any[][] = [
          ...
          ["Concepto", "Motos Combustión (Uds)", "Motos Eléctricas (Uds)", "Total Motos (Uds)", "Monto ($)", "Presupuesto ($)", "% Cumplimiento"],
          ["Ventas Contado (Artículos)", "-", "-", "-", totalArticulosContado, "-", "-"],
          ["Ventas Contado (Motos)", totalMotosContadoCombUds, totalMotosContadoElecUds, totalMotosContadoUds, totalMotosContadoVal, "-", "-"],
          ["Ventas Crédito (Artículos)", "-", "-", "-", totalArticulosCredito, "-", "-"],
          ["Ventas Crédito (Motos)", totalMotosCreditoCombUds, totalMotosCreditoElecUds, totalMotosCreditoUds, totalMotosCreditoVal, "-", "-"],
          ["Total General Motos", totalMotosCombUds, totalMotosElecUds, totalMotosAllUds, totalMotosAllVal, "-", "-"],
          ["Total Ventas Artículos", "-", "-", "-", totalArticulosAll, globalSalesBudget, pctGlobalSales],
          [],
          ["Balance de Ventas por Vendedor"],
          ["Vendedor", "Motos Comb. (uds)", "Motos Eléc. (uds)", "Total Motos (uds)", "Total Artículos ($)", "Presupuesto ($)", "% Cump."]
        ];

        const empVentasData = Object.values(empSummary).filter(e => e.artContado > 0 || e.artCredito > 0 || e.motosContVal > 0 || e.motosCredVal > 0 || e.salesBudget > 0);
        if (empVentasData.length > 0) {
          empVentasData.forEach(e => {
            const tArt = e.artContado + e.artCredito;
            const pctS = e.salesBudget > 0 ? ((tArt / e.salesBudget) * 100).toFixed(1) + '%' : '0%';
            const mCombUds = e.motosContCombUds + e.motosCredCombUds;
            const mElecUds = e.motosContElecUds + e.motosCredElecUds;
          ...
          sheetRows.push(["Sin ventas registradas en el periodo", 0, 0, 0, 0, 0, "0%"]);
        }

        sheetRows.push([]);
        sheetRows.push(["Balance de Cobranzas por Cobrador"]);
        sheetRows.push(["Cobrador", "Total Cobrado ($)", "Presupuesto ($)", "% Cump."]);

        const empCobrosData = Object.values(empSummary).filter(e => e.collections > 0 || e.collBudget > 0);
        if (empCobrosData.length > 0) {
          empCobrosData.forEach(e => {
            const pctC = e.collBudget > 0 ? ((e.collections / e.collBudget) * 100).toFixed(1) + '%' : '0%';
            sheetRows.push([
              e.name,
              e.collections,
          ...
          sheetRows.push(["Sin cobros registrados en el periodo", 0, 0, "0%"]);
        }

        // Continuación solicitada en Excel: Evolución de Cartera por Cobrador (Inicio vs Fin de Mes)
        sheetRows.push([]);
        sheetRows.push([`Evolución de Carteras por Cobrador (Inicio de Mes vs. Fin de Mes: ${reportCutoffs.initialDate || reportStartDate} al ${reportCutoffs.closingDate || reportEndDate})`]);
        sheetRows.push([
          "Cobrador", 
          "Cartera Inicio ($)", 
          "Cartera Fin ($)", 
          "Variación / Recuperación ($)", 
          "% Variación", 
          "Inicio Al Día (0-30 d)", 
          "Fin Al Día (0-30 d)", 
          "Inicio Vencida (>30 d)", 
          "Fin Vencida (>30 d)",
          "Inicio (0 d)", 
        ...
        if (excelPortfolioRows.length > 0) {
          excelPortfolioRows.forEach(row => sheetRows.push(row));
          sheetRows.push([
            "TOTAL CONSOLIDADO",
            totalInit,
            totalClose,
          ...
          sheetRows.push(["Sin cortes de cartera registrados en el periodo"]);
        }

        const wsGlobal = XLSX.utils.aoa_to_sheet(sheetRows);
...
function CommerceCard({ data }: { data: any; key?: any }) {
  ...
  const pe = data.portfolioEvolution as CollectorPortfolioEvolution | undefined;

  return (
    <div className="bg-white dark:bg-neutral-900 rounded-2xl border border-neutral-200 dark:border-neutral-800 p-6 shadow-sm flex flex-col justify-between">
      <div>
        ...
        <div className="space-y-6">
          ...
          {canCollect && (
            <div className="space-y-2">
              <div className="flex justify-between items-end">
                <div>
                  <p className="text-xs font-semibold text-neutral-500 uppercase tracking-wider">Cobranzas del Periodo</p>
                  <div className="flex items-baseline gap-2">
          ...
          {/* PORTFOLIO EVOLUTION SUMMARY (OPCIÓN A - COMPACTA) */}
          {(canCollect || pe?.hasData) && (
            <div className="pt-3 border-t border-neutral-100 dark:border-neutral-800 space-y-3">
              <div className="flex items-center justify-between">
                ...
                {pe?.initialDate && pe?.closingDate && (
                  <span className="text-[10px] font-medium text-neutral-500 dark:text-neutral-400 bg-neutral-100 dark:bg-neutral-800 px-2 py-0.5 rounded-full font-mono">
                    {pe.initialDate} → {pe.closingDate}
                  </span>
                )}
              </div>
              ...
              {pe && pe.hasData ? (
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <div className="p-2.5 rounded-xl bg-neutral-50 dark:bg-neutral-800/60 border border-neutral-100 dark:border-neutral-800">
                      <p className="text-[10px] uppercase font-bold text-neutral-400 tracking-wider">Inicio Mes</p>
                      <p className="text-sm font-extrabold text-neutral-900 dark:text-neutral-100 font-mono mt-0.5">
                        ${pe.initialAmount.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                      </p>
                      <p className="text-[10px] text-neutral-500 dark:text-neutral-400 mt-0.5">
                        {pe.initialClients} clientes
                      </p>
                    </div>

                    <div className="p-2.5 rounded-xl bg-neutral-50 dark:bg-neutral-800/60 border border-neutral-100 dark:border-neutral-800">
                      <p className="text-[10px] uppercase font-bold text-neutral-400 tracking-wider">Fin / Actual</p>
                      <p className="text-sm font-extrabold text-neutral-900 dark:text-neutral-100 font-mono mt-0.5">
                        ${pe.closingAmount.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                      </p>
                      <p className="text-[10px] text-neutral-500 dark:text-neutral-400 mt-0.5">
                        {pe.closingClients} clientes
                      </p>
                    </div>

                    <div className={cn(
                      "p-2.5 rounded-xl border",
                      pe.variationAmount <= 0
                      ...
                      <p className="text-[10px] uppercase font-bold text-neutral-500 dark:text-neutral-400 tracking-wider">
                        {pe.variationAmount <= 0 ? 'Recuperado' : 'Variación'}
                      </p>
                      <p className={cn(
                        "text-sm font-extrabold font-mono mt-0.5 flex items-center gap-0.5",
                        pe.variationAmount <= 0 ? "text-emerald-700 dark:text-emerald-300" : "text-amber-700 dark:text-amber-300"
                      )}>
                        {pe.variationAmount <= 0 ? (
                          <>
                            <TrendingDown className="w-3.5 h-3.5 inline" />
                            -${pe.recoveredAmount.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                          </>
                        ) : (
                          <>
                            <TrendingUp className="w-3.5 h-3.5 inline" />
                            +${pe.variationAmount.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                          </>
                        )}
                      </p>
                      <p className="text-[10px] font-bold mt-0.5 text-neutral-600 dark:text-neutral-400">
                        {pe.initialAmount > 0 ? `${pe.variationPct >= 0 ? '+' : ''}${pe.variationPct.toFixed(1)}%` : '0%'}
                      </p>
                    </div>
                  </div>

                  {/* Portfolio Quality: On-Time vs Overdue */}
                  <div className="p-3 bg-neutral-50/80 dark:bg-neutral-800/40 rounded-xl border border-neutral-100 dark:border-neutral-800 text-xs space-y-1.5">
                    <div className="flex justify-between items-center">
                      <span className="text-neutral-600 dark:text-neutral-400 font-medium">Al Día (0-30 d):</span>
                      <div className="flex items-center gap-1.5 font-mono text-neutral-900 dark:text-neutral-100 font-bold">
                        <span className="text-neutral-500 font-normal">${pe.initialOnTimeAmount.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
                        <span className="text-neutral-400">→</span>
                        <span>${pe.closingOnTimeAmount.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
                      </div>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-neutral-600 dark:text-neutral-400 font-medium">Vencida (&gt;30 d):</span>
                      <div className="flex items-center gap-1.5 font-mono font-bold">
                        <span className="text-neutral-500 font-normal">${pe.initialOverdueAmount.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
                        <span className="text-neutral-400">→</span>
                        <span className={pe.closingOverdueAmount <= pe.initialOverdueAmount ? "text-emerald-600 dark:text-emerald-400" : "text-red-500 dark:text-red-400"}>
                          ${pe.closingOverdueAmount.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                        </span>
                      </div>
                    </div>
                  </div>

                  {/* Expandable Aging Brackets Breakdown */}
                  <div>
                    <button
                      type="button"
                      onClick={() => setShowBrackets(!showBrackets)}
                      className="w-full flex items-center justify-between text-[11px] font-bold text-neutral-500 hover:text-neutral-800 dark:text-neutral-400 dark:hover:text-neutral-200 py-1 transition-colors"
                    >
                      <span className="flex items-center gap-1">
                        <Layers className="w-3 h-3" />
                        {showBrackets ? 'Ocultar desglose por tramos de mora' : 'Ver desglose por tramos de mora (Inicio vs Cierre)'}
                      </span>
                      {showBrackets ? <ChevronUp className="w-3.5 h-3.5" /> : <ChevronDown className="w-3.5 h-3.5" />}
                    </button>

                    {showBrackets && (
                      <div className="mt-2 border border-neutral-200 dark:border-neutral-800 rounded-xl overflow-hidden animate-in fade-in slide-in-from-top-1 text-xs">
                        <table className="w-full">
                          <thead className="bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-300 text-[10px] uppercase font-bold">
                            <tr>
                              <th className="py-1.5 px-2 text-left">Tramo</th>
                              <th className="py-1.5 px-2 text-right">Inicio</th>
                              <th className="py-1.5 px-2 text-right">Cierre</th>
                              <th className="py-1.5 px-2 text-right">Var.</th>
                            </tr>
                          </thead>
                          <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800 bg-white dark:bg-neutral-900">
                            {AGING_BRACKETS.map(b => {
                              const initB = pe.initialBrackets[b.key];
                              const closeB = pe.closingBrackets[b.key];
                              const initAmt = initB?.totalAmount || 0;
                              const closeAmt = closeB?.totalAmount || 0;
                              const diff = round2(closeAmt - initAmt);
                              return (
                                <tr key={b.key} className="hover:bg-neutral-50/60 dark:hover:bg-neutral-800/40">
                                  <td className="py-1.5 px-2 font-medium text-neutral-700 dark:text-neutral-300">
                                    {b.label}
                                  </td>
                                  <td className="py-1.5 px-2 text-right font-mono text-neutral-500">
                                    ${initAmt.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                                  </td>
                                  <td className="py-1.5 px-2 text-right font-mono font-bold text-neutral-900 dark:text-neutral-100">
                                    ${closeAmt.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                                  </td>
                                  <td className={cn(
                                    "py-1.5 px-2 text-right font-mono font-bold text-[11px]",
                                    diff <= 0 ? "text-emerald-600 dark:text-emerald-400" : "text-amber-600 dark:text-amber-400"
                                  )}>
                                    {diff <= 0 ? `-$${Math.abs(diff).toLocaleString('en-US', { minimumFractionDigits: 2 })}` : `+$${diff.toLocaleString('en-US', { minimumFractionDigits: 2 })}`}
                                  </td>
                                </tr>
                              );
                            })}
                ...
                <div className="p-3 bg-neutral-50 dark:bg-neutral-800/50 rounded-xl text-center text-neutral-400 text-xs italic border border-dashed border-neutral-200 dark:border-neutral-700">
                  Sin cortes de cartera registrados en el periodo para este cobrador.
                </div>
              )}
            </div>
          )}
